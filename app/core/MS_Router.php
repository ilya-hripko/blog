 ...
 */
class MS_Router extends CI_Router
{
    // --------------------------------------------------------------------
    ...
    {
        // added by Ilya H.
        // this array for not rout to PS controller (for ajax)
        $not_use=$this->config->item('is_public_not_use');
        if($this->config->item('is_public')&&(!in_array($segments[0], $not_use)))
            array_unshift($segments, 'public_site');
        ...
        if(file_exists(APPPATH . 'controllers/' . $segments[0] . EXT))
        {
            ...
            if(($segments[0] == 'cabinet' && !$xhr)  || $segments[0] != 'cabinet') {
                return $segments;
            }
        }
